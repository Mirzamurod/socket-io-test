const io = require('socket.io')(5005, { cors: { origin: '*', methods: ['GET', 'POST'] } })
...
io.on('connection', socket => {
  ...
  socket.on('createContact', ({ currentUser, receiver }) => {
    ...
    if (receiverSocketId) socket.to(receiverSocketId).emit('getCreatedUser', currentUser)
  })
  ...
  socket.on('readMessages', ({ receiver, messages }) => {
    const receiverSocketId = getSocketId(receiver._id)
    if (receiverSocketId) socket.to(receiverSocketId).emit('getReadMessages', messages)
  })
  ...
  socket.on('typing', ({ receiver, sender, message }) => {
    const receiverSocketId = getSocketId(receiver._id)
    if (receiverSocketId) socket.to(receiverSocketId).emit('getTyping', { sender, message })
  })
